...
**Status: experimental.** ~585 lines. The automated painted-frame matrix is Chromium-only — its runners drive CDP, which no other engine speaks — but the scenarios self-drive and print their read-out on the page, so the touch/gesture class is verified on WebKit by hand as well: desktop Safari over WebDriver, the iOS Simulator, and on-device probes on a real iPhone. Not production software; the receipts below say exactly what has and hasn't been proven.
...
- **Touch write-gate + spacer absorption**: while a touch gesture owns the viewport — finger down, plus a settle window kept alive by the gesture's own scroll events, so it spans momentum — the machinery does not write `scrollTop` at all. A write cancels an in-flight fling wherever momentum belongs to the scroll view, and near the bottom per-event restores deadlock a slow drag outright (measured: 599/600 frames reverted, 8px travelled of 4800). Corrections arriving during the gesture are absorbed by the **top spacer** instead (`gestureAdj`) — the second lever flow layout provides and an absolute-positioned design does not, since rows placed by transform can only be compensated in `scrollTop`, which is exactly what a gesture forbids. The gate is on touch events with no UA test: platforms whose momentum lives in the input stream (a macOS trackpad, arriving as wheel events) emit no touch events at all, so gating on touch *is* the platform test. Release restates the reference frame from **what is painted** rather than from the model, which is the one thing a spacer clamped at a boundary can make disagree.

## Receipts (same corpus, same machine)
...
Touch/device axes — WebKit (desktop Safari over WebDriver, iOS Simulator, real iPhone), before → after the touch write-gate:

| Axis | before | after |
|---|---|---|
| Slow drag off the bottom (600 frames) | 599 frames reverted, 8px travelled of 4800 | **0 reverted, full travel** |
| Drift under the finger (48,000px travel) | ±46–90px per mount batch | **0 events, 0px** |
| Painted jump at gesture flush | up to +148px | **0–1px** |
| Simulator fling reach | ~2 messages (momentum killed) | **~39 messages** |
| Release at the top of history | −182px painted shift | **0px** |
| Release mid-history | — | 435px of scrollTop paid, **1px** painted |

The bottom deadlock reads identically in Blink and WebKit (599/600 both), so it was virtualizer behaviour rather than an engine difference; the other rows are WebKit-specific, since Blink on a desktop trackpad has no fling for a `scrollTop` write to cancel. For contrast on the same phone and corpus, TanStack replays its deferred-adjustment ledger after the gesture as a single **3,675px** settle jump — the ledger is what the spacer removes, not the gate.

Three arms, one simulator (iPhone 16 Pro, iOS 26.5), same corpus and the same scripted finger — `?touch=1`, which counts every `scrollTop` write while the finger is down and every sampled frame that moves against it, no row identity involved (`docs/RESULTS.md` § 12):

| Axis | ballast ro | LegendList web | TanStack |
|---|---|---|---|
| `scrollTop` writes while the finger is down (slow drag into cold history) | **0** | 3–7 per drag (`scrollAdjustBy`) | 0 (iOS-gated deferral) |
| Frames moving against the finger | **0** | 1 per drag, 34–410px | 0 |
| Fling after a 500px flick | **1,632px / 3.0s** | 168px, dead in <300ms | 1,045px / 3.0s |
| Write at release | 1 (spacer fold, 213px; picture ±1px per § 9) | — | 1 ledger replay (370px; 1,932px once, after a lost `touchend`) |

The same three arms on desktop WebKit, driven as a 600-frame `scrollTop` walk at finger speed framed by synthetic touch events, count 0 / 43 / 37 library writes respectively — the write *count* is engine-agnostic; only what the engine does with a write differs.
...
| Measured on this corpus | 0% every axis, both gears; 0 writes under a finger | 0.1% scroll-up, 1.4–1.7% stream, 0% CDP gestures (Blink); on WebKit touch 3–7 `scrollTop` writes per slow drag, fling dead at 168px | 11.1% @ ≤10px scroll-up, 0.2% stream, 0% CDP gestures; on iOS writes deferred under the finger, replayed at release |
| Size / deps | ~585 lines, zero deps | RN+web dual-platform codebase | multi-framework engine (core + adapters) |
...
**vs LegendList web.** The closest relative — its mvcp anchoring philosophy is one of the three ideas this design borrows, it measures gesture-clean under Blink's CDP drags, and it is what the author's own production app ships today. Under WebKit *touch* it does not (`docs/RESULTS.md` § 12, the class [legend-list#488](https://github.com/LegendApp/legend-list/issues/488) reports): its anchoring writes `scrollTop` 3–7 times per slow drag into cold history via `scrollAdjustBy`, one frame per drag moves against the finger (34–410px), and a 500px flick's momentum dies at 168px where ballast's runs 1,632px — the write-gate + spacer absorption is exactly the mechanism it lacks. ballast's other edges are narrower and honestly stated: the streaming margin (0.1–0.2% vs 1.4–1.7%, structural — same-task measure+restore has no observable window); the sync gear's zero-proof (a reference implementation showing artifacts are a pipeline choice, which a shipped library cannot afford to carry); auditability (~585 lines vs a dual-platform RN+web codebase, relevant when a design system wants an ownable primitive rather than a dependency); and document-flow semantics (native cross-row selection, find-in-page, cheap aria-posinset — absolute positioning fights all three). LegendList wins on production maturity, one API across RN and web, recycling and per-type estimates, and a maintained feature surface. One caveat applies to both it and TanStack equally: neither guards paint-level behavior in CI (LegendList ships jsdom-only tests; TanStack's own benchmarks missed its regression) — which is why this repo treats the conformance harness as half the deliverable.
...
- **Cross-engine coverage is uneven, and none of it is checked in**: every percentage in the first table is a Blink number, because the runners in `bench/` drive CDP and nothing else speaks it. The engine-sensitive class — touch — is covered off that path: the bottom-deadlock probe reads identically in Blink and WebKit (599/600 frames reverted before the fix, both engines), momentum cancellation and the gesture flush were measured in the iOS Simulator and on a real iPhone, Gecko is spot-checked on two axes through an ad-hoc Playwright script (`docs/RESULTS.md` § 7, § 8), and four of the five instrument errors collected in § 10 were caught by hand on the device rather than by anything on the bench. So the engines have been *looked at*; what does not exist is a non-Chromium run this repo can re-run — the Playwright path was never committed, and one-off runs do not guard a regression. Folding it into `bench/` and extending it to Playwright's WebKit is the obvious next step; virtua's e2e suite (real WebKit, touch/momentum emulation, per-browser tolerances) is the model.
- **OPEN — an over-large estimate blanks the viewport for most of a drag**: at `?est=1200` on `mix=wild`, 2634 of 3595 dragged frames show a gap up to 796px of an 800px viewport, recovering 9 frames after release; under-estimates read 0. Cause is known (the gesture-time absorb takes its anchor through `paintOrigin()`, so under a systematic bias each pass corrects against a frame the previous pass moved and `gestureAdj` compounds — measured max 1.25M px). Four fixes were measured and all traded it for something worse; details in `docs/RESULTS.md` § 9.
- **A repricing move can outrun the position it owes, and nothing in-repo guards it**: a running-average move reprices every unmeasured row at once, so the pass that reprices owes a scrollTop correction for the whole delta above the anchor. Until 2026-08-26 the repricing decision sat inside `recompute()`'s O(N) row loop behind a clock gate, so a loop long enough to straddle the moment that gate opened priced the head of the list at the old number and the tail at the new one — the pass restored to that half-repriced offset, consumed `geoChanged`, and the next pass, holding the correct target, found the flag clear and never wrote. `anchorToKey` landed up to 698,000px short, permanently, in 25–50% of runs at 100k rows; traced, fixed, and verified in `docs/RESULTS.md` § 11 (the decision is now taken once per pass, and prices are frozen while a declaration is in flight — LegendList's mechanism; virtua instead reprices once and hands the caller the displacement to apply as a jump). What is NOT closed is the guard: this repo's harness cannot arm the reproduction, because its corpora settle the averages during mount, so the probe that actually fails on the old code (`bench/tanstack-suite/repriceprobe.mjs`) depends on TanStack's benchmark app. `bench/repriceprobe.mjs` checks the invariant against the harness and honestly reports `skip`. The harness owes a fast-mounting, uniform-row corpus.
- **O(N) per pass, so past ~50k rows this is the wrong primitive**: `recompute()` rebuilds the offsets and keys arrays plus a whole key→index Map on every pass. At 100k that is ~2.8× TanStack on jump-to-end settle (188–228ms vs 77–88ms) and ~2× its heap (57–77MB vs 28–32MB); at 30k it is still a tie (92–107ms vs 84–91ms), and at 10k the whole suite is parity. Correcting the estimate does not move it — this is the rebuild, not convergence. virtua's lazy prefix-sum (`computedOffsetIndex`, offsets materialised only up to the index actually asked for) is the reference fix and is not a small change. Chat transcripts are thousands of rows, so this is scope, not a bug — but it is the reason the 100k cells exist in § 11.
...
- `bench/` — CDP runners: scripted scenarios (`bench.mjs`), touch-gesture replay (`wheelbench.mjs`), live hand-scroll instrumentation (`livemeasure.mjs`), long-jump fill (`jump.mjs`), follow-semantics probes (`followprobe.mjs`, `nudgestorm.mjs`), the repricing invariant probe (`repriceprobe.mjs`), and debugging probes. `bench/tanstack-suite/` adapts ballast into TanStack Virtual's own Playwright benchmark suite — cost axes (mount, settle, `scrollToIndex` accuracy, heap), no painted artifacts — and carries the validated reproduction for § 11. CDP is Chromium-only, which is why the WebKit and on-device numbers come from the in-page probes instead: `?probe=1` (a `scrollTop` setter shadow plus `scrollTo`/`scrollBy`/`scrollIntoView` interception, separating JS writers from browser-initiated movement), `?blank=1`, `?release=1`, and `?touch=1` (per-gesture: library `scrollTop` writes under the finger, frames moving against it, fling travel — arm-agnostic, no row identity) render their read-out on the page, so a phone with no debugger attached still produces evidence. Desktop Safari is driven through `safaridriver --mcp` (Technology Preview, remote automation enabled), one tool call per measurement; the Gecko spot-checks used an ad-hoc Playwright script that is not checked in.
